...
Vec3f RayTracer::pixelColor(const Rayon3f &rayon, int depth, Vec3f &attenuation, ofstream& file) const
{
	if (depth <= 0)
    {
	...
	HitRecord rec;
	if (scene.world()->touche(rayon, 0, INFINITY, rec, file))
	{
		...
		if (rec.pMaterial && rec.pMaterial->scatter(rayon, rec, localAttenuation, vScattered, attenuation))
		{
			...
			{
				color+= pixelColor(scattered, depth - 1, attenuation, file).cwiseProduct(localAttenuation);
			}
        ...
        for (int j = 0; j < width; ++j)//, x+=coef)
        {
			...
			for (auto k = 0; k < antiAliasing; k++)
            {
				...
				for (auto l = 0; l < antiAliasing; l++)
				{
					...
					auto localPixel = pixelColor(r, 16, attenuation, myfile);
					pixel += localPixel;
